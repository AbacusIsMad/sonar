...
def image_update( Qdata, fig, Nrep, Nplot, stop_flag):
    ...
    while(  not stop_flag.is_set() ):
        ...
        img = np.roll( img, 1, 0); #rolls down by 1 row, as you would expect
        #rbga, probably
        view = img.view(dtype=np.uint8).reshape((Nrep, Nplot, 4))
        view[0,:,:] = cm.jet(new_line)*255; #I don't think this copies values.
    
        #source.data['image'] = [img]
        source.data.update({'image' : [img]})
        #print(view) #both img and view updates, and they should show correctly.
        #bk.output_notebook()
        #bk.show(fig,notebook_handle=True)
        push_notebook(handle=bk_notebook_handle) #but this isn't working??
        #print(bk_notebook_handle)
        Qdata.queue.clear();
...
def rtsonar( f0, f1, fs, Npulse, Nseg, Nrep, Nplot, maxdist, temperature, functions ):
    ...
    view = img.view(dtype=np.uint8).reshape((Nrep, Nplot, 4))
    view[:,:,3] = 255; #rgba, setting alpha value to full, others to 0
    ...
    global bk_notebook_handle
    bk_notebook_handle = bk.show(fig,notebook_handle=True) # add notebook_handle=True to make it update
    push_notebook(handle=bk_notebook_handle)
